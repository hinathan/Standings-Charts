...
var StandingsChart = {
	...
	init:function(inputs,canvas) {
		...
		if(this.opts.floatCenterline && !this.opts.rankOnly) {
			var max = this.arrayMax(totals);
			for(var i in totals) {
				if(totals[i] < max) {
					for(var j in item_column_min_max) {
						var corr = (max - totals[i])/3;
						item_column_min_max[j][i].max += corr;
						item_column_min_max[j][i].min += corr;
					}
				}
			}
