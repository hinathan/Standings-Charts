...
var StandingsChart = {
	...
	init:function(inputs,canvas) {
		...
		for(var i in item_column_min_max) {
			...
			texts.push({
				color:this.contrastColour(ctx.fillStyle),
				text:name,
				x:xs[xs.length-1].right - ctx.measureText(name).width - margin,
	...
	computeMetrics:function(data) {
		...
		for(i in data) {
			...
			width = (this.ctarget.width - (this.getXGutter() * (-1 + point_count))) / point_count;
